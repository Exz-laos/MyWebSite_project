...
app.get('/dashboard', async (req, res) => {
    try {
        ...
        for (let i = 1; i <= 12; i++) {
            ...
            let monthSum = 0; // Variable to accumulate the sum for the current month
            ...
            for (let j = 0; j < billSaleInMonth.length; j++) {
                ...
                monthSum += sum._sum.price ?? 0; // Accumulate sum for the month
            }

            // Push or update month sum in arr
            if (monthSum > 0 || i <= new Date().getMonth() + 1) { // Ensures no empty months are pushed
                arr.push({ month: i, sumPrice: monthSum });
            }
        }
